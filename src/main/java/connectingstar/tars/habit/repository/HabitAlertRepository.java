...
public interface HabitAlertRepository extends JpaRepository<HabitAlert,Integer> {
     ...
     * 2. RunHabit table JOIN.
     * 3. device table을 habitAlert.user_id = device.owning_user_id 조건으로 LEFT OUTER JOIN
     *
     * @author 이우진
     */
    ...
    List<HabitAlertWithDevice> findByAlertTimeBetweenWithRunHabitAndDevice(@Param("startTime") LocalTime startTime, @Param("endTime") LocalTime endTime);
}
